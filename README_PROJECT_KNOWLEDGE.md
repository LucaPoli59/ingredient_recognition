...
**Stato della ricognizione:** architettura e flusso principale verificati nel codice; il dataset Yummly è stato analizzato integralmente su metadata e 65.146 immagini. Le regole esplicite e testate hanno prodotto il baseline riproducibile `ingredients_target_v4_metadata.json` (161 etichette, 60.354 ricette), ma non è selezionato per il runtime: la sua soglia di frequenza elimina ingredienti validi prima dell'associazione semantica. È in corso la ricerca di un vocabolario controllato nel Work package 2.2c del piano Data.
...
Il piano esecutivo attivo della fase Data è `docs/plans/yummly_data_phase.md`. Copre i Work package 2.1b–2.4: store immagini condiviso, compatibilità degli esperimenti storici, standardizzatore `ingredients_target`, audit del vocabolario (2.2a), rafforzamento dell'estrattore (2.2b), ricerca del vocabolario controllato (2.2c), implementazione della nuova pipeline (2.2d), split deterministico con gruppi SHA-256 esatti e integrazione runtime senza `<UNK>` nei nuovi output multi-label. La compatibilità legacy (2.1c) resta deferred. Il baseline `v4` e le sue regole sono conservati come confronto riproducibile, ma non devono essere integrati: la nuova pipeline assocerà prima ogni ingrediente a un concetto del vocabolario, userà una standardizzazione minima solo come fallback, e applicherà le soglie ai concetti finali. Prima di modificare loader, layout Yummly, metadata o builder occorre leggere sia il piano generale sia questo piano esecutivo e mantenerne sincronizzati i tracker.
